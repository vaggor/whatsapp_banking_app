...
class Chat extends AppModel{
	...
	public function saveChat($cif,$name,$from,$to,$message,$type){
		...
		$data['Chat']['cif'] = $cif;
		$data['Chat']['name'] = $name;
		$data['Chat']['from_phone_no'] = $from;
		$data['Chat']['to'] = $to;
		$data['Chat']['message'] = $message;
		$data['Chat']['type'] = $type;
		$data['Chat']['date'] = date('Y-m-d H:i');
		$this->saveAll($data);
		return;
	}
	...
	public function getUser(){
		...
		foreach ($data as $data) {
			$resp[$i]['phone'] = explode('+', $data['Chat']['phone'])[1];
			$resp[$i]['name'] = $user->getFirstNameByPhoneNo($data['Chat']['phone']);
			$this->updateNewUserPulled($data['Chat']['phone']);
			$i++;
		}
		return $resp;
	}
